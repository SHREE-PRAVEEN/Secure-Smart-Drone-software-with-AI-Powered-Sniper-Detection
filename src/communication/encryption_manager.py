...
Utilities for symmetric AES-256-GCM encryption, key generation, on-disk key management,
and optional RSA wrapping/unwrapping for symmetric key exchange.

Usage:
    from encryption_manager import EncryptionManager

    mgr = EncryptionManager(key_dir='config/security/keys')
    # generate and save a new key (if none exists)
    key_id = mgr.generate_and_save_key()
    ct_b64 = mgr.encrypt(b"hello world", key_id=key_id, aad=b"mayuri-v1")
    pt = mgr.decrypt(ct_b64, key_id=key_id, aad=b"mayuri-v1")
"""
