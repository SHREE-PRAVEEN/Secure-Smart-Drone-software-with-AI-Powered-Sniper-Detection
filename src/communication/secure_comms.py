...
ROS2 node that provides a secure publish/subscribe bridge over the local encrypted mesh.

- Subscribes to:  /mayuri/outbound_msg  (std_msgs/String)  --> sends encrypted to mesh peers
- Publishes to:   /mayuri/inbound_msg   (std_msgs/String)  <-- messages received from mesh (decrypted JSON)
- Also publishes mesh peer list on /mayuri/peers (std_msgs/String as JSON)
- Uses EncryptionManager (AES-256-GCM) + MeshNode (UDP discovery/unicast)
- Safe-by-design: transmits JSON payloads only; does not perform any offensive actions.

Parameters (ROS2 params):
- node_name (string)        : friendly name for this mesh participant (default: "mayuri_node")
- key_dir (string)          : path to keys (default: "config/security/keys")
- key_id (string, optional) : specific AES key id to use (if omitted, first key or generated)
- broadcast_port (int)      : discovery broadcast port (mesh_network default is used if left)
- unicast_port (int)        : unicast port (mesh_network default is used if left)
- publish_peer_interval (double) : seconds between publishing peer list

Usage:
  ros2 run mayuri_communication secure_comms
  # or directly:
  python3 mayuri/src/communication/secure_comms.py
"""
...
class SecureCommsNode(Node):
    ...
    def on_outbound(self, msg: String):
        """
        Expects JSON string in msg.data. Example:
        {"to": "all", "type": "status", "body": {"battery": 85}}
        {"to": "192.168.1.10", "type":"cmd", "body": {...}}
        """
        try:
