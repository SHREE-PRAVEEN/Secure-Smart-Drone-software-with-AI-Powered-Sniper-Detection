...
ROS2 node that receives high-level JSON commands on /mayuri/command (std_msgs/String)
and translates them into MAVLink commands sent to an autopilot (UDP/TCP/Serial).

Intended for safe development and simulation (SITL) first. Test thoroughly before using with real vehicles.
"""
...
class CommandBridge(Node):
    ...
    def _takeoff(self, altitude_m):
        ...
        # Standard NAV_TAKEOFF: param7 = altitude (m)
        # Some firmwares expect guided/takeoff via set_mode + throttle; this is best-effort.
        self._send_command_long(mavutil.mavlink.MAV_CMD_NAV_TAKEOFF, param7=float(altitude_m))

    def _land(self):
        self.get_logger().info('Requesting LAND...')
        # NAV_LAND has no params generally required
    ...
    def _goto(self, lat, lon, alt):
        ...
        # Using MAV_CMD_NAV_WAYPOINT (param5=lat, param6=lon, param7=alt)
        # Some firmwares accept MAV_CMD_NAV_WAYPOINT; others might require SET_POSITION_TARGET_GLOBAL_INT
    ...
    def _set_mode(self, mode_str):
        ...
        # Best-effort method for setting mode: send MAV_CMD_DO_SET_MODE or use mav.set_mode (not always available)
        # We'll attempt a common approach: MAV_CMD_DO_SET_MODE with custom_mode integer 1..n is firmware-dependent.
        # For PX4, using mav.set_mode requires different handling; here we use COMMAND_LONG MAV_CMD_DO_SET_MODE as a safe attempt.
        try:
            # param1 = base_mode (set 1 for custom), param2 = custom_mode (firmware dependent)
            # It's safer to request the user to set mode via ground station; here we log and attempt.
