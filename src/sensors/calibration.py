...
calibration.py
-------------------------------------------------
MAYURI Sensor Calibration System

Performs intrinsic & extrinsic calibration for cameras,
lidar, radar, and IMU systems to ensure accurate sensor fusion.

==> Intrinsic camera calibration (chessboard pattern)
==> Extrinsic sensor alignment (LIDAR ↔ camera ↔ radar)
==> Auto save/load calibration data
==> Real-time calibration check
==> ROS2 integrated calibration publisher
...
class CalibrationManager(Node):
    ...
    def perform_camera_calibration(self, image_files):
        ...
        for fname in image_files:
            ...
            if ret:
                ...
                self.get_logger().warning(f"⚠️ Chessboard not detected in {fname}")
    ...
    def load_calibration_data(self):
        ...
        try:
            ...
            self.get_logger().warning(f"⚠️ Failed to load calibration file: {e}")
