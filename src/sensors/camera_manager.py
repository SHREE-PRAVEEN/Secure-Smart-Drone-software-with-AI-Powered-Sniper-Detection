...
camera_manager.py
-------------------------------------------------
MAYURI Camera Management System

Handles all onboard and simulated camera streams (RGB, IR, Depth).
Interfaces with the AI core for real-time object detection and tracking.

==> Multi-camera support
==> Real-time frame streaming
==> ROS2 topic publishing for AI modules
==> Works with physical cameras or simulation videos
==> Auto recovery if a camera feed drops
    ...
    def _open_stream(self, source, cam_type):
        ...
        try:
            if self.simulation_mode and isinstance(source, str):
                self.get_logger().info(f"🎞️ Simulated {cam_type} source: {source}")
