...
path_planner.py
-------------------------------------------------
MAYURI Autonomous Path Planner

Computes optimal and safe navigation paths between waypoints,
using A* search over a dynamic occupancy grid.

==> Grid-based A* path planning
==> Dynamic obstacle avoidance (via sensor updates)
==> Re-routing on blocked paths
==> ROS2 integrated for mission planning
==> Works in simulation or real-world mode
...
# -------------------------------------------------------------
# Utility: A* Search Implementation
# -------------------------------------------------------------
class AStarPlanner:
    """Simple 2D A* grid planner."""

    def __init__(self, grid, resolution=1.0):
        self.grid = grid  # 2D list of 0 (free) / 1 (obstacle)
...
class PathPlanner(Node):
    ...
    def on_path_request(self, msg: String):
        ...
        try:
            ...
            self.get_logger().info(f"🗺️ Path request: {start} → {goal}")
            ...
            if not path:
                self.get_logger().warning("⚠️ No valid path found.")
