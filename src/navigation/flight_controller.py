...
ROS2 node that acts as the Flight Controller interface for MAYURI.

- Receives high-level flight commands on /mayuri/flight/cmd (std_msgs/String with JSON payload).
- Sends MAVLink commands to autopilot (if use_mavlink=True) or simulates behavior.
- Publishes status on /mayuri/flight/status and events on /mayuri/flight/event.

Safety & notes:
- Simulation-first. Always test with SITL (PX4/ArduPilot) before any physical flight.
- This code issues high-level MAVLink commands (COMMAND_LONG, SET_POSITION_TARGET_*). Specific autopilot behavior may vary.
- Do NOT use this node to control weapons or conduct unauthorized activities.
"""
...
# Optional MAVLink dependency
try:
    from pymavlink import mavutil
except Exception:
    mavutil = None  # handle if not installed
