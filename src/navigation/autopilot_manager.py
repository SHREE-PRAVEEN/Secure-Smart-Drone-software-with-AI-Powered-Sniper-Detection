...
-------------------------------------------------
MAYURI Autonomous Flight Coordinator

Responsible for:
- Interpreting high-level commands (from command_bridge or mission_planner)
- Managing autopilot states and flight modes
- Supervising system health, link status, and failsafes
- Publishing vehicle status to other nodes (UI, telemetry, AI)

==> Safe and ROS2-compatible
==> Works with PX4/ArduPilot SITL via command_bridge.py
==> Simulation-first design (no direct motor control)
"""
...
class AutopilotManager(Node):
    ...
    def hold_position(self):
        ...
        self.get_logger().info("⏸️ Holding position.")
        ...
        while self.running and rclpy.ok():
            ...
            if now - self.last_heartbeat > self.heartbeat_timeout:
                if self.status["link_active"]:
                    ...
                    self.get_logger().warning("⚠️ Telemetry link lost.")
